...
use msm_scoring_domain::{
    ...
    ports::{ClvEndpoint, DataLayerRevenue, ExplainEndpoint, ConfigSource, PredictionSink, PredictionRecord, PortError},
};
use msm_scoring_infrastructure::{
    BigQueryDataLayer, PubSubPredictions, VertexClvEndpoint, VertexExplain,
    SecretManagerConfig,
};

fn features() -> ClickFeatures {
    ClickFeatures::try_new(ClickFeaturesInput {
        click_id: "c-adapt".into(), correlation_id: "t".into(),
...
async fn stub_token(server: &MockServer) {
    Mock::given(method("GET")).and(path("/token"))
        .respond_with(ResponseTemplate::new(200).set_body_json(
            json!({"access_token": "fake", "expires_in": 3600})))
        .mount(server).await;
}
...
async fn vertex_explain_parses_attributions() {
    ...
    Mock::given(method("POST")).and(path("/explain"))
...
async fn vertex_explain_errors_on_non_2xx() {
    ...
    Mock::given(method("POST")).and(path("/explain"))
        ...
        .mount(&server).await;
    let ep = VertexExplain::new(format!("{}/explain", server.uri()), Duration::from_secs(2));
    assert!(matches!(ep.explain(&features()).await, Err(PortError::Upstream(_))));
}
...
async fn vertex_clv_parses_prediction() {
    ...
    std::env::set_var("GCP_METADATA_TOKEN_URL", format!("{}/token", server.uri()));
    stub_token(&server).await;
    Mock::given(method("POST")).and(path("/clv"))
...
async fn vertex_clv_rejects_missing_prediction_field() {
    ...
    Mock::given(method("POST")).and(path("/clv"))
        ...
        .mount(&server).await;
    let ep = VertexClvEndpoint::new(format!("{}/clv", server.uri()), Duration::from_secs(2));
    assert!(matches!(ep.predict(&features()).await, Err(PortError::Upstream(_))));
}
...
async fn bigquery_data_layer_sums_jobs_query_rows() {
    ...
    std::env::set_var("GCP_METADATA_TOKEN_URL", format!("{}/token", server.uri()));
    stub_token(&server).await;
    Mock::given(method("POST")).and(path_regex(r".*/projects/proj/queries$"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "rows": [{"f": [{"v": "12.50"}]}]
        })))
        .mount(&server).await;
    let bq = BigQueryDataLayer::with_api_root(
        server.uri(), "proj".into(), "ds".into(), "tbl".into(), Duration::from_secs(2),
    );
...
async fn pubsub_predictions_publishes_message_payload() {
    ...
    std::env::set_var("GCP_METADATA_TOKEN_URL", format!("{}/token", server.uri()));
    stub_token(&server).await;
    Mock::given(method("POST")).and(path_regex(r".*/topics/t-int:publish$"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({"messageIds": ["1"]})))
        .mount(&server).await;
    let sink = PubSubPredictions::with_api_root(
        server.uri(), "proj".into(), "t-int".into(), Duration::from_secs(2),
    );
    sink.record(PredictionRecord {
        click_id: "c-1".into(), correlation_id: "t".into(),
        predicted_rpc: 2.5, source: "MODEL",
        model_version: "v1".into(), ts_ms: 1,
    }).await.unwrap();
}
...
async fn pubsub_predictions_surfaces_non_2xx() {
    ...
    std::env::set_var("GCP_METADATA_TOKEN_URL", format!("{}/token", server.uri()));
    stub_token(&server).await;
    Mock::given(method("POST")).and(path_regex(r".*:publish$"))
        .respond_with(ResponseTemplate::new(503))
        .mount(&server).await;
    let sink = PubSubPredictions::with_api_root(
        server.uri(), "proj".into(), "t".into(), Duration::from_secs(2),
    );
    assert!(sink.record(PredictionRecord {
        click_id: "c".into(), correlation_id: "t".into(),
        predicted_rpc: 1.0, source: "MODEL",
        model_version: "v".into(), ts_ms: 1,
    }).await.is_err());
}
...
async fn secret_manager_config_initial_fetch_returns_typed_values() {
    ...
    Mock::given(method("GET")).and(path_regex(r".*/secrets/.*:access$"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
