...
fn features() -> ClickFeatures {
    ClickFeatures::try_new(ClickFeaturesInput {
        click_id: "c-int".into(), correlation_id: "t-int".into(),
...
fn build_uc(vertex_url: String) -> ScoreClick {
    let deps = ScoreClickDeps {
        model: Arc::new(VertexEndpoint::new(vertex_url, "v-int".into(), Duration::from_millis(200))),
...
async fn vertex_happy_path_returns_model_source() {
    ...
    Mock::given(method("POST")).and(path("/predict"))
...
async fn vertex_timeout_trips_breaker_and_falls_back() {
    ...
    Mock::given(method("POST")).and(path("/predict"))
        .respond_with(ResponseTemplate::new(200)
            ...
            .set_delay(Duration::from_millis(500))) // exceeds 200ms timeout
        .mount(&server).await;
