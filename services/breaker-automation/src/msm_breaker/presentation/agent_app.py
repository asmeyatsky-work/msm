...
Adapters are built only when the data env is present, so the package imports
without GCP credentials (root_agent=None on bare import).
"""
...
if _PROJECT and _DATASET:
    _signal_source = MonitoringSignalSource(_PROJECT, _DATASET)
    ...
    _signal_source = _notifier = root_agent = _triage = None


async def run_triage(event: AnomalyEvent) -> TriageOutput:
    """Run the triage agent for one anomaly and dispatch the escalation.

    The schema gate (§4) is `TriageOutput` validation + `to_domain()`; the
    escalation write happens in `TriageAnomaly`.
    """
    if root_agent is None or _triage is None:
    ...
    sid = str(event.occurred_at_ms)
    await session_service.create_session(
        app_name="breaker-triage", user_id="breaker", session_id=sid,
    )
    ...
    async for _ in runner.run_async(user_id="breaker", session_id=sid, new_message=prompt):
        pass
    ...
    output = TriageOutput.model_validate(raw if isinstance(raw, dict) else json.loads(raw))
